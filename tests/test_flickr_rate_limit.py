...
def test_rate_limit_retries_with_delays_then_raises(mocker):
    ...
    flickr_api_mock = cast(MagicMock, bot.flickr_api)
    flickr_api_mock.get_single_photo_info.side_effect = make_429_error()
    ...
    assert flickr_api_mock.get_single_photo_info.call_count == 4
...
def test_rate_limit_succeeds_on_retry(mocker):
    ...
    photo = MagicMock()
    flickr_api_mock = cast(MagicMock, bot.flickr_api)
    flickr_api_mock.get_single_photo_info.side_effect = [make_429_error(), photo]
